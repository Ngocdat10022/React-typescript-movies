...
function* handleGetMoviesNowPlaying(action: PayloadAction<any>): any {
...
function* handleGetMoviesTopRated(action: PayloadAction<any>): any {
  const { payload } = action;
...
function* handleGetMoviesPopular(action: PayloadAction<any>): any {
  const { payload } = action;
  console.log(payload.type);
  const res = yield call(requestMovies, { type: payload.type, page: 2 });
  const data = res.data.results;
  if (data) yield put(getMoviesPopularSuccess({ data: data }));
}

function* handleSearchMovies(action: PayloadAction<any>): any {
...
function* handleGetMoviesDetails(action: PayloadAction<any>): any {
  const { payload } = action;
...
function* handleGetMoviesCredits(action: PayloadAction<any>): any {
  const { payload } = action;
...
function* handleGetMoviesTrailer(action: PayloadAction<any>): any {
  const { payload } = action;
...
function* handleGetMoviesSimilar(action: PayloadAction<any>): any {
  const { payload } = action;
